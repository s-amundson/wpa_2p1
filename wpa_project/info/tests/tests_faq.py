...
class TestsFaq(TestCase):
    ...
    def test_get_faq_edit_good(self):
        self.client.get(reverse('info:edit'), secure=True)
    ...
    def test_get_faq_edit_bad(self):
        ...
        response = self.client.get(reverse('info:edit'), secure=True)
    ...
    def test_get_faq_edit_id(self):
        ...
        f.category.add(c)
        response = self.client.get(reverse('info:edit', kwargs={'faq_id': f.id}), secure=True)
        self.assertTemplateUsed('student_app/form_as_p.html')
    ...
    def test_post_faq_detail(self):
        ...
        response = self.client.post(reverse('info:edit'), d, secure=True)
        f = Faq.objects.all()
        self.assertRedirects(response, reverse('info:faq'))
        self.assertEqual(len(f), 1)
        self.assertEqual(f[0].question, 'test question')

    def test_get_faq_list(self):
        self.client.get(reverse('info:faq'), secure=True)
        self.assertTemplateUsed('info/faq_list.html')
