...
class TestsCosts(TestCase):
    ...
    def test_costs_new_cost(self):
        response = self.client.post(reverse('registration:costs'), self.costs, secure=True)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed('student_app/costs.html')
    ...
    def test_costs_edit_cost(self):
        response = self.client.get(reverse('registration:costs', kwargs={'cost_id': 1}), secure=True)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed('student_app/costs.html')
        response = self.client.post(reverse('registration:costs', kwargs={'cost_id': 1}), self.costs, secure=True)
        cm = CostsModel.objects.all()
        self.assertEqual(len(cm), 1)
        cm = cm[0]
        self.assertEqual(cm.name, "Test Cost")
