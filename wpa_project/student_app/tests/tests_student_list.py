...
class TestsSearchList(TestCase):
    ...
    def test_student_list_api_no_access(self):
        # Get the page, if not super or board, page is forbidden
        self.test_user = User.objects.get(pk=2)
        self.client.force_login(self.test_user)
        response = self.client.get(reverse('registration:student_list_json'), secure=True)
        self.assertEqual(response.status_code, 403)
    ...
    def test_student_list_access_api_allowed(self):
        response = self.client.get(reverse('registration:student_list_json'), secure=True)
        self.assertEqual(response.status_code, 200)
        logging.debug(str(response.content, encoding='utf8'))
        self.assertTrue(str(response.content, encoding='utf8').__contains__('"recordsTotal": 6'))
