...
# from https://github.com/docker-library/postgres/blob/master/14/bullseye/docker-entrypoint.sh added lines to do replication
...
_main() {
    ...
    echo $(id -u)
      ...
      if [ $PG_REPLICATE == 1 ]; then
        pg_basebackup --pgdata $PGDATA --format=p --write-recovery-conf --checkpoint=fast --label=mffb --progress --host=$PGHOST --port=$PGPORT --username=$PGUSER
      fi
    fi
    ...
    if [ "$(id -u)" = '0' ]; then
      ...
      if [ $PG_REPLICATE == 1 ]; then
        pg_basebackup --pgdata $DIR --format=p --write-recovery-conf --checkpoint=fast --label=mffb --progress --host=$PGHOST --port=$PGPORT --username=$PGUSER
			else
			  docker_init_database_dir
			fi
			pg_setup_hba_conf "$@"
